                ...
                >
                    {loading
                        ? Array.from({ length: 4 }).map((_, idx) => (
                            <div key={idx} className="snap-center shrink-0 w-[300px] md:w-[380px]">
                                <div className="h-[440px] rounded-[2.5rem] bg-gray-200 animate-pulse" />
                            </div>
                        ))
                        : categories.map((cat) => {
                            const tags = cat.tag
                                ? cat.tag.split(",").map((t) => t.trim()).filter(Boolean)
                                : [];
                            const resolvedBg = cat.backgroundImage
                                ? (cat.backgroundImage.startsWith('http') ? cat.backgroundImage : `${API_BASE_URL}${cat.backgroundImage}`)
                                : null;
                            ...
                            return (
                                <div key={cat.id} className="snap-center shrink-0 w-[300px] md:w-[380px] group">
                                    <a
                                        href={`/tuyen-dung?categorySlug=${cat.slug || cat.id}`}
                                    ...
                                    >
                                        ...
                                        <div className="relative z-10 h-full flex flex-col justify-end p-8">
                                            ...
                                            <p className="text-gray-300 text-sm leading-relaxed mb-6">
                                                {cat.description ?? "Khám phá các cơ hội việc làm hấp dẫn trong lĩnh vực này."}
                                            </p>
