...
import { formatSalary, workTypeLabel, timeAgo, companyInitial } from "@/lib/utils";
import type { PublicJobResponse } from "@/lib/types";

/* ── Color palette for logo fallback ── */
const FALLBACK_COLORS = [
    "bg-blue-600", "bg-orange-500", "bg-purple-600", "bg-teal-600",
    "bg-pink-600", "bg-emerald-600", "bg-rose-600", "bg-indigo-600",
];
        ...
        <div className="relative bg-gradient-to-r from-yellow-50 to-white p-1 rounded-2xl shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer group isolation-auto">
            ...
            >
                ...
                {job.contactStaff?.avatar ? (
                    ...
                    <div className={`w-20 h-20 rounded-2xl ${bgColor} text-white flex items-center justify-center font-black text-2xl shrink-0 shadow-lg border-2 border-white`}>
                        {initial}
                    </div>
                )}
    ...
    return (
        ...
        >
            {job.contactStaff?.avatar ? (
                ...
                <div className={`w-14 h-14 rounded-lg ${bgColor} text-white flex items-center justify-center font-bold text-lg shrink-0 shadow-md`}>
                    {initial}
                </div>
            )}
...
export default function JobListings({ keyword, location }: JobListingsProps) {
    ...
    const loadJobs = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await fetchLatestJobs({ limit: 20 });
            setJobs(data);
        } catch (err) {
    ...
    return (
        <div className="space-y-4">
            ...
            {/* VIP Jobs */}
            {!loading && !error && vipJobs.map((job) => (
                <VipJobCard key={job.id} job={job} />
            ))}

            {/* Normal Jobs — first batch */}
            {!loading && !error && normalJobs.slice(0, 3).map((job) => (
                <NormalJobCard key={job.id} job={job} />
            ))}
            ...
            {!loading && !error && normalJobs.slice(3).map((job) => (
                <NormalJobCard key={job.id} job={job} />
            ))}
            ...
            {!loading && !error && jobs.length > 0 && (
                <div className="flex justify-center pt-8">
                    <button className="px-6 py-2 border border-gray-300 rounded-full hover:bg-black hover:text-white transition-all font-medium cursor-pointer">
                        Xem thêm việc làm
                    </button>
                </div>
            )}
