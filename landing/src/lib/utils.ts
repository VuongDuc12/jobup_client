...
/**
 * Resolve media path to absolute URL.
 * - Absolute URL stays unchanged
 * - Relative path is prefixed by NEXT_PUBLIC_ASSET_BASE_URL (if provided)
 * - Otherwise use API base origin
 */
export function resolveAssetUrl(path?: string | null): string | null {
  ...
  const envBase = process.env.NEXT_PUBLIC_ASSET_BASE_URL?.trim();
  let apiOrigin = "";
